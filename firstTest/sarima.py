...
t_dm = dm
split_value = len(dm[kafsimo]) - periodos
...
dm['existing_forecast'] = existing_results.predict(start = split_value, end = len(dm[kafsimo]), dynamic= True)  
# dm[[kafsimo, 'existing_forecast']].plot(figsize=(12, 8))
# plt.show()
...
# future_model=sm.tsa.arima.ARIMA(dm[kafsimo],order=(1, f_d, 1))
...
p_start = len(dm[kafsimo])
p_end = len(dm[kafsimo]) + periodos
data['future_forecast'] = future_results.predict(start = p_start, end = p_end, dynamic= True) 

# data[[kafsimo, 'future_forecast']].plot(figsize=(12, 8))
# plt.show()

# fig1=plot_acf(dm[kafsimo].diff().dropna())
# fig2=plot_pacf(dm[kafsimo].diff().dropna())

# fig1=plot_acf(dm[kafsimo].diff(12).dropna())
# fig2=plot_pacf(dm[kafsimo].diff(12).dropna())
